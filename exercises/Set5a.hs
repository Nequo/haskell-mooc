...
totalPrice (MkShoppingEntry _ cost qty) = cost * fromIntegral qty
...
buyOneMore (MkShoppingEntry name cost qty) = MkShoppingEntry name cost (qty + 1)
...
tick :: UpDown -> UpDown
tick (Up val) = Up (val + 1)
tick (Down val) = Down (val - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Up val) = Down val
toggle (Down val) = Up val
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix color1 color2) = let avg x y = (x + y) / 2 in zipWith avg (rgb color1) (rgb color2)
  ...
  where
    ...
    toBinHelper n =
      let remainder = n `mod` 2
          nextDigits = toBinHelper (n `div` 2)
       in if remainder == 0
            then O nextDigits
            else I nextDigits
