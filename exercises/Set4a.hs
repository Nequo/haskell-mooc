...
rangeOf xs = let sorted = sort xs in last sorted - head sorted
...
longest' :: (Ord a) => [[a]] -> [a] -> [a]
longest' [] currentLongest = currentLongest
longest' (x : xs) currentLongest
  | length currentLongest > length x = longest' xs currentLongest
  | length currentLongest == length x = if head currentLongest >= head x then longest' xs x else longest' xs currentLongest
  | length currentLongest < length x = longest' xs x
...
incrementKey key xs = map (updateKey key) xs
  where
    updateKey key (x, y) = if x == key then (x, y + 1) else (x, y)
...
winner scores player1 player2 =
  let player1Score = Map.findWithDefault 0 player1 scores
   in let player2Score = Map.findWithDefault 0 player2 scores
       in (if player1Score >= player2Score then player1 else player2)
...
transfer from to amount bank
  | not (Map.member from bank) = bank -- from account doesn't exist
  | not (Map.member to bank) = bank -- to account doesn't exist
  | amount < 0 = bank -- amount is negative
  | fromBalance < amount = bank -- insufficient funds
  | otherwise = Map.insert to (toBalance + amount) $ Map.insert from (fromBalance - amount) bank
  where
    fromBalance = bank Map.! from -- balance of from account
    toBalance = bank Map.! to -- balance of to account
