...
sums :: Int -> [Int]
sums i = sumsHelper i 1 0

sumsHelper i current prevSum = if current > i then [] else (prevSum + current) : sumsHelper i (current + 1) (prevSum + current)
...
sumsOf' _ [] = []
sumsOf' prevSum (x : xs) = (prevSum + x) : sumsOf' (prevSum + x) xs
