...
capitalize s = unwords $ map caps $ words s
  where
    caps :: [Char] -> [Char]
    caps w = toUpper (head w) : tail w
...
joinToLength n xs = [x1 ++ x2 | x1 <- xs, x2 <- xs, length (x1 ++ x2) == n]
...
-- can be expressed as multiCompose = foldr (.) id
...
go x y [] = []
go x y (command : commands) = case command of
  "up" -> go x (y + 1) commands
  "down" -> go x (y - 1) commands
  "left" -> go (x - 1) y commands
  "right" -> go (x + 1) y commands
  "printX" -> show x : go x y commands
  "printY" -> show y : go x y commands
  _ -> go x y commands
