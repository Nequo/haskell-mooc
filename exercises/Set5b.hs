...
walk (step : steps) (Node _ left right) = case step of
  StepL -> walk steps left
  StepR -> walk steps right
...
set _ _ Empty = Empty -- Path falls off the tree
set (step : steps) val (Node nodeVal left right) = case step of
  StepL -> Node nodeVal (set steps val left) right
  StepR -> Node nodeVal left (set steps val right)
...
search val (Node v left right)
  | val == v = Just [] -- Found the value at current node
  | otherwise = case search val left of
      Just path -> Just (StepL : path) -- Found in left subtree
      Nothing -> case search val right of
        Just path -> Just (StepR : path) -- Found in right subtree
        Nothing -> Nothing -- Not found anywhere
