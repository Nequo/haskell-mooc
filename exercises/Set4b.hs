...
countHelper m count = maybe (count + 1) (const count) m

-- countHelper m count = case m of
-- Nothing -> count + 1
-- Just _ -> count
...
concatHelper :: [a] -> [a] -> [a]
concatHelper curr new = curr ++ new
...
largestHelper new (x : xs) = case compare new x of
  GT -> [new]
  EQ -> new : x : xs
  _ -> x : xs
